...
cursor.execute(
    "SELECT * FROM sqlite_master WHERE name ='FACES' and type='table';")
chk = cursor.fetchone()
if chk is not None:
...
while True:
    ...
    for (top, right, bottom, left), name in zip(face_locations, face_names):
        ...
        if name != "Unknown":
            ...
            cv2.putText(frame, 'Permission Granted !!', (int(
                width / 4), height - 50), font, 1.0, (255, 255, 255), 1, cv2.LINE_AA)
        else:
            cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 2)
            cv2.rectangle(frame, (left, bottom - 35),
                          (right, bottom), (0, 0, 255), cv2.FILLED)
            cv2.putText(frame, 'Permission Denied !!', (int(
                width / 4), height - 50), font, 1.0, (255, 255, 255), 1, cv2.LINE_AA)
